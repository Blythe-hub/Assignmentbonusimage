...
class ImageGraph:
    ...
    def create_adjacency_matrix(self):
        ...
        # Fill the adjacency matrix based on vertex edges
    ...
    def bfs(self, start_index, color):
        ...
        # Convert start_index to integer if it's a string
        if isinstance(start_index, str):
            start_index = int(start_index)
        
        # Get the original color of the starting vertex
        original_color = self.vertices[start_index].color
        
        # If the starting vertex already has the target color, no need to proceed
        ...
        # Visit and color the starting vertex
        self.vertices[start_index].visit_and_set_color(color)
        self.print_image()  # Print after coloring for visualization
        ...
        while not queue.is_empty():
            ...
            for neighbor_index in self.vertices[current_index].edges:
                neighbor = self.vertices[neighbor_index]
                
                # If neighbor is not visited and has the original color
                if not neighbor.visited and neighbor.color == original_color:
                    # Visit and color this neighbor
    ...
    def dfs(self, start_index, color):
        ...
        while not stack.is_empty():
            ...
            # Find an unvisited neighbor with the original color
            ...
            # If no unvisited neighbor is found, backtrack by popping from stack
...
def create_graph(data):
    ...
    for i in range(num_vertices):
        vertex_info = lines[line_index].split(', ')
        x = int(vertex_info[0])
        y = int(vertex_info[1])
        color = vertex_info[2]
        
        # Create and add the vertex to the graph
    ...
    for i in range(num_edges):
        edge_info = lines[line_index].split(', ')
        from_index = int(edge_info[0])
        to_index = int(edge_info[1])
    ...
    search_info = lines[line_index].split(', ')
    start_index = int(search_info[0])
    search_color = search_info[1]
    
    # Return the ImageGraph, starting position, and color as a tuple
...
def main():
    ...
    # Print adjacency matrix in a readable format
