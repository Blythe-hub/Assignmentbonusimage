...
class ImageGraph:
    ...
    def bfs(self, start_index, color):
        ...
        you may choose to call print_image in this method debugging yourself
        ...
        while not queue.is_empty():
            ...
            for nbr in self.vertices[current].edges:
                v = self.vertices[nbr]
    ...
    def dfs(self, start_index, color):
        ...
        Do not remove the first 2 statements we provide.
        you may choose to call print_image in this func method debugging yourself
...
def create_graph(data):
    ...
    num_v = int(lines[idx]); idx += 1
    graph = ImageGraph(image_size)
    for i in range(num_v):
        x_s, y_s, col = [p.strip() for p in lines[idx].split(",")]
        x, y = int(x_s), int(y_s)
        graph.vertices.append(ColoredVertex(i, x, y, col))
        idx += 1
    num_e = int(lines[idx]); idx += 1
    for _ in range(num_e):
        u_s, v_s = [p.strip() for p in lines[idx].split(",")]
        u, v = int(u_s), int(v_s)
        graph.vertices[u].add_edge(v)
        graph.vertices[v].add_edge(u)
        idx += 1
    start_s, fill_col = [p.strip() for p in lines[idx].split(",")]
    start_idx = int(start_s)
    return graph, start_idx, fill_col
...
def main():
    ...
    """
    data = sys.stdin.read()
    graph, start_idx, fill_col = create_graph(data)
    matrix = graph.create_adjacency_matrix()
    print("Adjacency Matrix:")
    for row in matrix:
        print(row)
    graph.bfs(start_idx, fill_col)
    graph, start_idx, fill_col = create_graph(data)
    graph.dfs(start_idx, fill_col)
